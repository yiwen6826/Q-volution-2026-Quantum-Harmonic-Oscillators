...
def make_dir(k:int, omega:float, y0:float, vy0:float, bx:float, by:float) -> str:
    ...
    """
    # by convention round to one decimal
    stem = f"k{k}_w{omega:.1f}_y{y0:.1f}_vy{vy0:.1f}_bx{bx:.1f}_by{by:.1f}"
    
    return os.path.join("data", stem)


def data_exists(k:int, omega:float, y0:float, vy0:float, bx:float, by:float) -> bool:
...
def save_data(k:int, omega:float, y0:float, vy0:float, bx:float, by:float, time:np.array, data_expected:dict, data_actual:dict) -> None:
